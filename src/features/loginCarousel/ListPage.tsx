...
import { publishLoginCarousel } from "./publish";
...
export function LoginCarouselPage() {
  const { t } = useTranslation();
  const queryClient = useQueryClient();
  const itemsQuery = useQuery({ queryKey: ["login-carousel"], queryFn: () => loginCarouselRepo.list() });
  const { setPublishAction } = usePageActions();
  const { isViewer } = useRole();

  const [draftItems, setDraftItems] = React.useState<LoginCarouselItem[]>([]);
  const [isDirty, setIsDirty] = React.useState(false);
  const [deleteId, setDeleteId] = React.useState<string | null>(null);
  const [publishOpen, setPublishOpen] = React.useState(false);
  const [isPublishing, setIsPublishing] = React.useState(false);
  const [editItem, setEditItem] = React.useState<LoginCarouselItem | null>(null);
  ...
  const handleReorder = (ids: string[]) => {
    const reordered = ids
      ...
      .filter((item): item is LoginCarouselItem => Boolean(item))
  ...
  const handleSaveDraft = async () => {
    await loginCarouselRepo.replaceAll(draftItems);
    await queryClient.invalidateQueries({ queryKey: ["login-carousel"] });
  ...
  const handlePublish = async () => {
    ...
    try {
      ...
      const snapshot = await publishLoginCarousel();
  ...
  const submitAdd = addForm.handleSubmit((values) => {
    ...
    const newItems: LoginCarouselItem[] = values.items.map((item, index) => ({
      id: uuid(),
  ...
  return (
    <div className="space-y-6">
      ...
      {draftItems.length === 0 ? (
        ...
        <div className="grid gap-6 lg:grid-cols-[1fr_320px]">
          ...
          <div className="rounded-lg border border-slate-200 bg-white p-4">
            <PhonePreview title={t("preview.login")}>{preview}</PhonePreview>
          </div>
        </div>
      )}
      ...
      <PublishDialog
        ...
        version={(getLatestSnapshot("LOGIN_CAROUSEL")?.version ?? 0) + 1}
